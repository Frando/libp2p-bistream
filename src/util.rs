...
pub trait MatchProtocol {
    fn has_protocol(&self, matcher: impl FnMut(Protocol) -> bool) -> bool;
}

impl MatchProtocol for Multiaddr {
    fn has_protocol(&self, matcher: impl FnMut(Protocol) -> bool) -> bool{
        self.iter().any(matcher)
    }
}

// Copied from:
// https://github.com/libp2p/rust-libp2p/blob/5755942d7742f9d0432c346318e25da504b65370/protocols/autonat/src/behaviour.rs#L554
...
impl GlobalIp for Multiaddr {
    ...
    fn is_private_ip(&self) -> bool {
        match self.iter().next() {
            Some(Protocol::Ip4(a)) => a.is_private(),
            _ => false,
        }
    }
}
